...
class ErrorPageState extends State<ErrorPage> with AutomaticKeepAliveClientMixin {
  ...
  void didUpdateWidget(covariant ErrorPage oldWidget) {
    ...
    if (widget.httpInteraction.error?.error != oldWidget.httpInteraction.error?.error) {
      _checkError();
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return ListView.separated(
      ...
      itemCount: widget.children.length + 2 + (widget.httpInteraction.error?.additionalData != null ? 1 : 0),
        ...
        switch (index - widget.children.length) {
            ...
            if (widget.httpInteraction.error != null) {
              if (isEncodable) {
                ...
                return ExpandableCard(
                  title: const Text("Error"),
                  expanded: true,
                  child: SelectableText(widget.httpInteraction.error!.error!.toString()),
                );
              }
            }
          ...
          case 1:
            return ExpandableCard(
              ...
              child: widget.httpInteraction.error?.stackTrace?.toString().isNotEmpty ?? false
                  ? SelectableText(widget.httpInteraction.error!.stackTrace.toString())
                  : null,
            );
          case 2:
            return BodyItem(
