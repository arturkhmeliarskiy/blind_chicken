  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(builder: (context, constraints) {
      if (constraints.maxWidth > 1100 && widget.entries.length > 1) {
        var firstList = widget.entries.sublist(0, (widget.entries.length / 2).ceil());
          ...
          child: Row(
            ...
            children: [
              Expanded(
                child: Column(
                  children: [
                    for (int i = 0; i < firstList.length; i++) ...buildItem(i, firstList),
                  ],
                ),
              ),
              ...
              Expanded(
                child: Column(
                  children: [
                    for (int i = 0; i < secondList.length; i++) ...buildItem(i, secondList),
                  ],
                ),
              ),
            ],
        ...
        return SingleChildScrollView(
          child: Column(
            children: [
              for (int i = 0; i < widget.entries.length; i++) ...buildItem(i, widget.entries),
            ],
          ),
        );
      }
