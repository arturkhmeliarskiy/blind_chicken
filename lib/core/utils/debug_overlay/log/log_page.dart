...
import 'package:pimmer_app/app/presentation/widgets/app_bars/base_app_bar.dart';
import 'package:pimmer_app/app/presentation/widgets/others/line.dart';
import 'package:pimmer_app/app/presentation/widgets/others/screen_wrapper.dart';
...
class _LogPageState extends State<LogPage> with AutomaticKeepAliveClientMixin, FilterCapability, SearchCapability {
        ...
        stream = stream.where((e) {
          for (String query in searchQueries) {
            bool match = [
              ...
              e.error,
              e.time,
            ].any((element) => element?.toString().toLowerCase().contains(query) ?? false);
            if (!match) {
              return false;
            }
          }
      ...
      showBarModalBottomSheet(
        ...
        builder: (BuildContext _) {
          return ScreenWrapper(
            child: Column(
              children: [
                const BaseAppBar(title: 'Log Details'),
                Expanded(
                  child: Padding(
                    padding: const EdgeInsets.all(8.0),
                    child: LogDetailsPage(entry: entry),
                  ),
                ),
              ],
            ),
    ...
    return ListView.separated(
      ...
      itemBuilder: (context, index) {
        ...
        return LogItem(
          ...
          entry: logEvent,
          selected: split && currentEntry != null ? currentEntry == logEvent : false,
          onSelected: () {
