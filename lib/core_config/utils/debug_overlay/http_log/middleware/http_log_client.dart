...
import 'package:blind_chicken/core_config/utils/debug_overlay/http_log/http_interaction.dart';
import 'package:blind_chicken/core_config/utils/debug_overlay/util/http_bucket.dart';
...
class HttpLogClient extends http.BaseClient {
  ...
  Future<http.StreamedResponse> send(http.BaseRequest request) async {
    ...
    try {
      ...
      if (enabled) {
        ...
        effectiveRequest = _copyRequest(request, http.ByteStream.fromBytes(requestBody));
      }
  ...
  HttpRequest convertRequest(http.BaseRequest request, Uint8List body) {
    ...
    return HttpRequest(
      ...
      body: Utils.isMediaTypeText(mediaType) ? http.Response.bytes(body, 200).body : body,
      time: DateTime.now(),
      additionalData: {
